...
X_u_train = np.vstack([X_lb, X_ub])  # (300,2)

Eu_icbc = np.vstack([Eu_lb, Eu_ub])  # (300,1)
Ev_icbc = np.vstack([Ev_lb, Ev_ub])
pu_icbc = np.vstack([pu_lb, pu_ub])
pv_icbc = np.vstack([pv_lb, pv_ub])
etau_icbc = np.vstack([etau_lb, etau_ub])
...
def output_transform(x, y):
    ...
    T1 = t_lower
    I = 1j
    E_true = torch.exp(((3 * X) / 2 - (65 * T1) / 8) * I) * (
        -1
        - 4
        * (-162 * I * T1 / 17 - 1)
        / (
            (16384 * (-(19 * T1) / 128 + (17 * X) / 64) ** 2) / 289
            + 1
            + (26244 * T1**2) / 289
        )
    )
    p_true = (
        ...
        * I
        / 17
        * (
            -(146088055 * X * T1**3) / 33554432
            - (1586899 * X**3 * T1) / 8388608
            + (12033042425 * T1**4) / 268435456
            + (1419857 * X**4) / 16777216
            + (134257551 * X**2 * T1**2) / 33554432
            - (355843677 * T1**2) / 134217728
            + (1252815 * X**2) / 33554432
            - (7767453 * T1 * X) / 33554432
            + 1085773 / 268435456
            - 622796445 * I * T1**3 / 33554432
            - 6765201 * I * T1 * X**2 / 8388608
            + 5595907 * I * T1 / 33554432
            + 83521 * I * X / 4194304
            + 7561107 * I * T1**2 * X / 8388608
        )
    ...
    eta_true = (
        4624 * X**4
        - 10336 * X**3 * T1
        + (218616 * T1**2 + 6664) * X**2
        + (-237880 * T1**3 + 158440 * T1) * X
        + 2449225 * T1**4
        - 136934 * T1**2
        - 799
    ) / (1565 * T1**2 - 76 * T1 * X + 68 * X**2 + 17) ** 2
    Eu_true = real(E_true)
    Ev_true = imag(E_true)
    pu_true = real(p_true)
    pv_true = imag(p_true)
    aaa = (1 - torch.exp(t_lower - T)) * Eu + Eu_true
    bbb = (1 - torch.exp(t_lower - T)) * Ev + Ev_true
    ccc = (1 - torch.exp(t_lower - T)) * pu + pu_true
    ddd = (1 - torch.exp(t_lower - T)) * pv + pv_true
    eee = (1 - torch.exp(t_lower - T)) * eta + eta_true
