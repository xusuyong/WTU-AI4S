...
X, T = np.meshgrid(x, t)
I = 1j
EExact = np.exp(((3 * T) / 2 - (65 * X) / 8) * I) * (
    -1
    - 4
    * (-162 * I * X / 17 - 1)
    / (
        (16384 * (-(19 * X) / 128 + (17 * T) / 64) ** 2) / 289
        + 1
        + (26244 * X**2) / 289
    )
)
pExact = (
    ...
    * I
    / 17
    * (
        -(146088055 * T * X**3) / 33554432
        - (1586899 * T**3 * X) / 8388608
        + (12033042425 * X**4) / 268435456
        + (1419857 * T**4) / 16777216
        + (134257551 * T**2 * X**2) / 33554432
        - (355843677 * X**2) / 134217728
        + (1252815 * T**2) / 33554432
        - (7767453 * X * T) / 33554432
        + 1085773 / 268435456
        - 622796445 * I * X**3 / 33554432
        - 6765201 * I * X * T**2 / 8388608
        + 5595907 * I * X / 33554432
        + 83521 * I * T / 4194304
        + 7561107 * I * X**2 * T / 8388608
    )
    * np.exp(((3 * T) / 2 - (65 * X) / 8) * I)
) / (
    4 * ((19 * X) / 128 - (17 * T) / 64) ** 2 + (6561 * X**2) / 1024 + 289 / 4096
) ** 2
etaExact = (
    4624 * T**4
    - 10336 * T**3 * X
...
lambda1 = dde.Variable(3.0)  # dtype = float64
lambda2 = dde.Variable(0.0)
...
def Eu_func(x):
    return (
        648 * np.sin(-(3 * x[:, 1:2]) / 2 + (65 * x[:, 0:1]) / 8) * x[:, 0:1]
        + (
            -1565 * x[:, 0:1] ** 2
            + 76 * x[:, 0:1] * x[:, 1:2]
            - 68 * x[:, 1:2] ** 2
        ...
        * np.cos(-(3 * x[:, 1:2]) / 2 + (65 * x[:, 0:1]) / 8)
    ) / (1565 * x[:, 0:1] ** 2 - 76 * x[:, 0:1] * x[:, 1:2] + 68 * x[:, 1:2] ** 2 + 17)


def Ev_func(x):
    return (
        (1565 * x[:, 0:1] ** 2 - 76 * x[:, 0:1] * x[:, 1:2] + 68 * x[:, 1:2] ** 2 - 51)
        * np.sin(-(3 * x[:, 1:2]) / 2 + (65 * x[:, 0:1]) / 8)
        + 648 * np.cos(-(3 * x[:, 1:2]) / 2 + (65 * x[:, 0:1]) / 8) * x[:, 0:1]
    ) / (1565 * x[:, 0:1] ** 2 - 76 * x[:, 0:1] * x[:, 1:2] + 68 * x[:, 1:2] ** 2 + 17)
...
def pu_func(x):
    return (
        (
            9796900 * x[:, 0:1] ** 4
            - 951520 * x[:, 1:2] * x[:, 0:1] ** 3
            + (874464 * x[:, 1:2] ** 2 - 579432) * x[:, 0:1] ** 2
            + (-41344 * x[:, 1:2] ** 3 - 50592 * x[:, 1:2]) * x[:, 0:1]
            + 18496 * x[:, 1:2] ** 4
            + 8160 * x[:, 1:2] ** 2
            + 884
        )
        ...
        * (
            x[:, 0:1] ** 3
            - (76 * x[:, 0:1] ** 2 * x[:, 1:2]) / 1565
            + ((68 * x[:, 1:2] ** 2) / 1565 - 1139 / 126765) * x[:, 0:1]
            - (136 * x[:, 1:2]) / 126765
        )
        * np.cos(-(3 * x[:, 1:2]) / 2 + (65 * x[:, 0:1]) / 8)
    ) / (
...
def eta_func(x):
    return (
        4624 * x[:, 1:2] ** 4
        - 10336 * x[:, 1:2] ** 3 * x[:, 0:1]
        + (218616 * x[:, 0:1] ** 2 + 6664) * x[:, 1:2] ** 2
        + (-237880 * x[:, 0:1] ** 3 + 158440 * x[:, 0:1]) * x[:, 1:2]
        + 2449225 * x[:, 0:1] ** 4
        - 136934 * x[:, 0:1] ** 2
        - 799
    ) / (
