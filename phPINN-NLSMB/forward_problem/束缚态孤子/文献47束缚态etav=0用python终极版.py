...
"""matlab要从底部看，初始（第一行）"""
idx_x = np.random.choice(nx, 150, replace=False)  # 输出数组x的行数输出为(0——256-1)，False无放回抽样
...
"""边界"""
idx_t = np.random.choice(nt, 150, replace=False)  # 从t中随机抽样50个数据
tb = t[idx_t, :]  # （50，1）
...
fig5.tight_layout()  # 自动调整大小和间距，使各个子图标签不重叠
...
ax.view_init(elevation, azimuth)
fig6.tight_layout()  # 自动调整大小和间距，使各个子图标签不重叠
...
ax.view_init(elevation, azimuth)
fig7.tight_layout()  # 自动调整大小和间距，使各个子图标签不重叠
...
# # ax.grid(False)#关闭背景的网格线
# ax.set_xlabel('$z$'); ax.set_ylabel('$t$'); ax.set_zlabel('$|E(t,z)|$')
# # fig8.colorbar(surf, shrink=0.5, aspect=5)
# ax.view_init(elevation, azimuth)
# fig8.tight_layout()#自动调整大小和间距，使各个子图标签不重叠
#
...
#                        antialiased=False)  # 抗锯齿
# # ax.grid(False)#关闭背景的网格线
# ax.set_xlabel('$z$'); ax.set_ylabel('$t$'); ax.set_zlabel('$|p(t,z)|$')
# # fig9.colorbar(surf, shrink=0.5, aspect=5)
# ax.view_init(elevation, azimuth)
# fig9.tight_layout()#自动调整大小和间距，使各个子图标签不重叠
#
#
...
#                        antialiased=False)  # 抗锯齿
# # ax.grid(False)#关闭背景的网格线
# ax.set_xlabel('$z$'); ax.set_ylabel('$t$'); ax.set_zlabel('$|\eta(t,z)|$')
# # fig10.colorbar(surf, shrink=0.5, aspect=5)
# ax.view_init(elevation, azimuth)
# fig10.tight_layout()#自动调整大小和间距，使各个子图标签不重叠
