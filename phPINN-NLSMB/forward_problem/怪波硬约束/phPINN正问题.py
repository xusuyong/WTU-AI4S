...
def output_transform(XT, y):
    ...
    aaa = (1 - exp(z - z_upper)) * (1 - exp(z_lower - z)) * (
        1 - exp(t_lower - t)
    ) * Eu + Eu_true
    bbb = (1 - exp(z - z_upper)) * (1 - exp(z_lower - z)) * (
        1 - exp(t_lower - t)
    ) * Ev + Ev_true
    ccc = (1 - exp(z - z_upper)) * (1 - exp(z_lower - z)) * (
        1 - exp(t_lower - t)
    ) * pu + pu_true
    ddd = (1 - exp(z - z_upper)) * (1 - exp(z_lower - z)) * (
        1 - exp(t_lower - t)
    ) * pv + pv_true
    eee = (1 - exp(z - z_upper)) * (1 - exp(z_lower - z)) * (
        1 - exp(t_lower - t)
    ) * eta + etaExact
