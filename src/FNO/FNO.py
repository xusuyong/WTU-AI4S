...
data_all = torch.from_numpy(sio.loadmat("NLS.mat")["uu_all"].astype("float32"))  # torch.Size([1600, 128, 200])
x_train = data_all[:ntrain, ::sub, :T_in]  # torch.Size([1400, 128, 100])前50个时间
y_train = data_all[:ntrain, ::sub, T_in : T + T_in]  # torch.Size([1400, 128, 100])后50个时间
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        optimizer.zero_grad()
        out = model(x).reshape(batch_size, ny, nx)
    ...
    writer.add_scalar("Loss/train", train_l2, ep)
    writer.add_scalar("Loss/test", test_l2, ep)

    if ep % step_size == 0 or ep == epochs - 1:
        torch.save(model, f"{folder}/bright_one_ep{ep}.pt")

        x = x_normalizer.decode(x.squeeze())
        idx = 6
        input = x[idx].squeeze().detach().cpu().numpy()
        truth = y[idx].squeeze().detach().cpu().numpy()
        pred = out[idx].squeeze().detach().cpu().numpy()

        plo(input, truth, pred, idx)
        idx = 10
        input = x[idx].squeeze().detach().cpu().numpy()
        truth = y[idx].squeeze().detach().cpu().numpy()
        pred = out[idx].squeeze().detach().cpu().numpy()
