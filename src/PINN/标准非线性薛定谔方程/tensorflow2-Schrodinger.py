...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, v0, tb, X_f, layers, lb, ub, learning_rate):
    ...
    def train(self, nIter):
        ...
        for it in range(nIter):
            ...
            if it % 100 == 0:
                ...
                start_time = time.time()
                MSE_history1.append(loss_value)
...
if __name__ == "__main__":
    ...
    model = PhysicsInformedNN(x0, u0, v0, tb, X_f, layers, lb, ub, learning_rate)

    start_time = time.time()
    MSE_hist1 = model.train(iter)
    elapsed = time.time() - start_time
    ...
    V_pred = v_pred.reshape(nt, nx)
    H_pred = h_pred.reshape(nt, nx)

    FU_pred = f_u_pred.reshape(nt, nx)
    FV_pred = f_v_pred.reshape(nt, nx)
    ...
    t1 = 0.1
    index = round((t1 - lb[1]) / A * (nt - 1))
    plt.plot(x, Exact_h[:, index], "b-", linewidth=2, label="Exact")
    ...
    plt.plot(
        ...
        "r-",
        linewidth=1,
        label="learning rate=%s" % (learning_rate),
    )  # + 1前面的数就是迭代次数
